...
class DatabaseHandler:
    ...
    async def insert_message_data(self, data: Dict[str, Any]) -> bool:
        # CORREÇÃO AQUI: Alterado para usar placeholders %s
        insert_query = """
    ...
    async def insert_messages_batch(
        ...
        for data_item in messages_data: # Renomeado 'data' para 'data_item' para evitar sombreamento
            if await self.insert_message_data(data_item):
                inserted_count += 1
